...
class AppAlertDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Dialog(
      ...
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(Dimens.radiusSmall)),
      child: Padding(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            Text(title ?? '',
                ...
                style: AppFontTextStyles.textStyleBold().copyWith(color: AppColors.regularTextColor),
                overflow: TextOverflow.visible),
            if (message.isNotBlank)
              Text(message!,
                  textAlign: TextAlign.start,
                  style: AppFontTextStyles.textStyleBold()
                      .copyWith(color: AppColors.primaryBlue1, fontSize: Dimens.fontSizeFourteen),
                  overflow: TextOverflow.visible),
            const Gap(Dimens.spaceMedium),
            Row(
              children: [
                if (negativeButtonTitle.isNotBlank)
                  Expanded(
                      child: _CommonButton(
                          title: negativeButtonTitle!, onTap: onNegativeTap ?? () => context.pop())),
                if (negativeButtonTitle.isNotBlank) const Gap(Dimens.spaceMedium),
                Expanded(
                    child: _CommonButton(
...
class _CommonButton extends StatelessWidget {
  ...
  const _CommonButton({required this.title, required this.onTap, this.isWarning = false});
  ...
  Widget build(BuildContext context) {
    return AppTextButton(
        ...
        textColor: isWarning ? AppColors.white : AppColors.primaryBlue1,
